...
Verified correct under **AddressSanitizer** and **UndefinedBehaviorSanitizer**
(no leaks, no out-of-bounds access, no UB).
...
```sh
make test     # build and run the correctness suite (49 checks)
...
src/
  tests.cpp                 correctness suite (no external framework)
...
allocator interface and `rebind`, and measuring/avoiding external fragmentation.
